...
export default function Dashboard() {
  ...
  return (
    <div className='flex flex-col flex-1 gap-8 sm:gap-12 md:gap-16'>
      <div className='grid grid-cols-3 bg-indigo-50 text-[#8f5ecc] p-4 gap-4 rounded-lg'>
        ...
        {Object.keys(moods).map((mood, moodIndex) => {
          return (
            <button onClick={() => {
              ...
              <p className={'text-[#8f5ecc] text-xs sm:text-sm md:text-base ' + fugaz.className}>{mood}</p>
            </button>
          )
        })}
      </div>
